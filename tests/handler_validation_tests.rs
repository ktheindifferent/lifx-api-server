...
mod handler_validation_tests {
    ...
    fn test_color_validation_through_state_update() {
        ...
        for color in valid_colors {
            ...
            assert!(
                result.is_ok(),
                "Should accept color '{}', but got error: {:?}",
                color,
                result.err()
            );
    ...
    fn test_invalid_combinations() {
        ...
        let invalid_cases = vec![
            ...
            (
                r#"{"selector": "all", "brightness": 1.5}"#,
                "brightness > 1.0",
            ),
            (
                r#"{"selector": "all", "infrared": -0.5}"#,
                "negative infrared",
            ),
            (
                r#"{"selector": "all", "duration": -1.0}"#,
                "negative duration",
            ),
            (
                r#"{"selector": "all", "brightness": "NaN"}"#,
                "NaN brightness",
            ),
            (
                r#"{"selector": "all", "infrared": "Infinity"}"#,
                "Infinity infrared",
            ),
        ];
    ...
    fn test_all_fields_optional_except_selector() {
        ...
        assert!(
            result.is_ok(),
            "Should accept StateUpdate with only selector"
        );
