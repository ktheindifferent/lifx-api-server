...
fn test_service_resilience_under_network_failures() {
                ...
                Err(e) => {
                    ...
                    if consecutive_errors >= max_consecutive_errors {
                        ...
                        let backoff_multiplier =
                            2_u32.saturating_pow(consecutive_errors.saturating_sub(1));
    ...
    assert!(
        errors_encountered.load(Ordering::Relaxed) > 0,
    ...
    worker_handle.join().expect("Failed to join worker thread");

    println!("Test completed successfully:");
    println!(
        "  Errors encountered: {}",
        errors_encountered.load(Ordering::Relaxed)
    );
    println!(
        "  Packets received: {}",
...
fn test_recovery_after_network_restoration() {
    ...
    for i in 0..3 {
        client
            .send_to(format!("packet{}", i).as_bytes(), server_addr)
    ...
    assert!(
        packets_received.load(Ordering::Relaxed) >= 4,
