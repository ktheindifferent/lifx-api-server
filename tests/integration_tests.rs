...
#[cfg(test)]
mod tests {
    use lifx_api_server::set_states::{
        SetStatesHandler, StateUpdate, StatesRequest, StatesResponse,
    };
    ...
    fn test_validation_brightness_out_of_range() {
        ...
        assert!(
            request.states[0]
                .brightness
                .expect("Brightness should be present in test state")
                > 1.0
        );
    }
    ...
    fn test_validation_infrared_out_of_range() {
        ...
        assert!(
            request.states[0]
                .infrared
                .expect("Infrared should be present in test state")
                < 0.0
        );
    }
