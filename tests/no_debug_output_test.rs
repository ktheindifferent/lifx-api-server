...
fn no_println_in_production_code() {
    let src_files = vec!["src/lib.rs", "src/main.rs", "src/set_states.rs"];
...
fn verify_logging_initialized() {
    ...
    let main_contents = fs::read_to_string("src/main.rs").expect("Failed to read main.rs");
...
fn verify_log_dependencies() {
    ...
    let cargo_contents = fs::read_to_string("Cargo.toml").expect("Failed to read Cargo.toml");
