...
fn test_concurrent_bulb_updates_stress() {
        ...
        let handle = thread::spawn(move || {
            for _ in 0..100 {
                match bulbs_clone.lock() {
                    ...
                    Err(e) => {
                        eprintln!(
                            "Reader thread {} encountered poisoned mutex: {}",
                            thread_id, e
                        );
        ...
        let handle = thread::spawn(move || {
            for i in 0..50 {
                match bulbs_clone.lock() {
                    ...
                    Err(e) => {
                        eprintln!(
                            "Writer thread {} encountered poisoned mutex: {}",
                            thread_id, e
                        );
                        // In production, this would return an error response
                        break;
    ...
    assert!(
        final_bulbs.len() >= 10,
        "Should have at least initial bulbs"
    );
    assert!(
...
fn test_high_contention_mutex_access() {
    ...
    match shared_data.lock() {
        Ok(guard) => {
            ...
            assert!(
                guard.len() >= expected_min as usize,
        ...
        Err(_) => {
            ...
            assert!(
                errors > 0,
                "Should have recorded errors if mutex is poisoned"
            );
        }
    };
}
...
fn test_rate_limiter_under_load() {
    ...
    assert!(
        total_success >= 50,
        "Should have at least 50 successful requests"
    );
    assert!(
        total_failed >= 50,
        "Should have at least 50 rate-limited requests"
    );
}
